"""
DeepAgent v2.0 - PESQUISA WEB REAL (Versão Compatível)
...
class DeepAgentWebSearch(BaseAgent):
    """
    ...
    def __init__(self, llm=None):
        super().__init__(
            name="DeepAgent", 
            description="Pesquisa web REAL v2.0"
        )
        ...
        if self.web_search_enabled:
            logger.info("🌐 DeepAgent v2.0 com DuckDuckGo Search inicializado!")
        else:
            # Fallback para Anthropic se disponível
            try:
                self._inicializar_cliente_anthropic()
                self.web_search_enabled = True
                logger.info("🌐 DeepAgent v2.0 com Anthropic inicializado!")
...
Responda de forma estruturada com análise baseada em dados reais."""
    ...
    def processar(self, mensagem: str, contexto: Optional[Dict] = None) -> str:
        """Processa comando para o DeepAgent v2.0"""
        try:
...
📋 **Principais Resultados:**"""
    ...
    def _resposta_ajuda(self) -> str:
        ...
        return f"""🌐 **DEEPAGENT v2.0 - PESQUISA WEB GRATUITA**
...
def criar_deep_agent_websearch(llm=None) -> DeepAgentWebSearch:
    """Cria DeepAgent v2.0 com Web Search"""
    return DeepAgentWebSearch(llm=llm)

# Alias para compatibilidade
criar_deep_agent = criar_deep_agent_websearch

# ===== TESTE =====

if __name__ == "__main__":
    print("🧪 Testando DeepAgent v2.0...")
    agent = criar_deep_agent_websearch()
